...
2. **Reference the shared logo with a relative path going up two levels:**
   `../../assets/logo.png`
   (Same pattern the file-converter tool uses — copy its `<head>` and topbar
...
4. **Register the tool in the hub.** Open `index.html` at the project root and
   ...
   const TOOLS = [
     {
       name: 'File Converter',
       desc: 'Convert PowerPoint ⇄ PDF, images, Word docs, and text between formats',
       path: 'tools/file-converter/index.html',
       enabled: true,
       icon: '<path d="M8 3 4 7l4 4"/><path d="M4 7h16"/><path d="M16 21l4-4-4-4"/><path d="M20 17H4"/>'
     },
